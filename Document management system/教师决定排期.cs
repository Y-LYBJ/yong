...
{
    public partial class 教师决定排期 : Form
    {
        教师主页面 w1 = new();
        public 教师决定排期()
        {
        ...
        public void NewSql()
        {
            try
            {
                ...
                conn.Open();
                string sql = "SELECT article,organization,type,nowRank FROM Article WHERE status = '录取' and RankState = '已拟定'";
        ...
        private void BtnOk_Click(object sender, EventArgs e)
        {
            if (this.dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (MessageBox.Show("确定决定" + article + "的排期吗", "注意", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        conn.ConnectionString = "Data Source=192.168.1.6,1433;Initial Catalog=\"management system\";Integrated Security=True;User ID = sa;pwd = 123456";
                        conn.Open();
                        string sql = "UPDATE Article SET RankState = '已确定', nowRank = '" + comboBox1.Text + "' WHERE article = '" + article + "'";
                        SqlCommand cmd = new SqlCommand(sql);
                        cmd.Connection = conn;
                        cmd.ExecuteNonQuery();
                        conn.Close();
                        MessageBox.Show("确定排期成功", "提示");
                        NewSql();
                    }
                    catch { MessageBox.Show("确定排期失败", "警告"); }
                }
            }
            else { MessageBox.Show("选中为空"); }
        }
        ...
        private void OK_Click(object sender, EventArgs e)
        {
            ...
            {
                string article = dataGridView1.CurrentRow.Cells[0].Value.ToString();
                if (MessageBox.Show("确定决定将" + article + "重新分配到" + comboBox1.Text + "吗", "注意", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    try
                    {
                        SqlConnection conn = new SqlConnection();
                        conn.ConnectionString = "Data Source=192.168.1.6,1433;Initial Catalog=\"management system\";Integrated Security=True;User ID = sa;pwd = 123456";
                        conn.Open();
                        string sql = "UPDATE Article SET nowRank = '" + comboBox1.Text + "' WHERE article = '" + article + "'";
                        SqlCommand cmd = new SqlCommand(sql);
                        cmd.Connection = conn;
                        cmd.ExecuteNonQuery();
                        conn.Close();
                        MessageBox.Show("修改排期成功", "提示");
                        NewSql();
                    }
                    catch { MessageBox.Show("修改排期失败", "警告"); }
